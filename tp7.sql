...
--1
SELECT numeroCoureur, nomCoureur FROM coureurs WHERE codePays IS NULL;
--2
SELECT COUNT(*) AS nombre_total_coureur, COUNT(codePays) AS nombre_coureur_avec_pays FROM coureurs;
--3
SELECT t.numeroEtape, c.numeroCoureur, c.nomCoureur FROM temps t JOIN coureurs c ON t.numeroCoureur = c.numeroCoureur ORDER BY t.numeroEtape;
--4
SELECT COUNT(*) FROM coureurs;
--5
SELECT AVG(EXTRACT(epoch FROM tempsRealise)/3600) AS temps_moyen, SUM(e.nbKm) AS total_km FROM temps t JOIN etapes e ON t.numeroEtape = e.numeroEtape WHERE t.numeroCoureur = 31;
--6
SELECT numeroEtape, nbKm FROM etapes ORDER BY nbKm DESC LIMIT 1;
--7
SELECT c.nomCoureur FROM coureurs c JOIN temps t ON c.numeroCoureur = t.numeroCoureur WHERE t.numeroEtape = 2 AND t.tempsRealise = (SELECT MIN(tempsRealise) FROM temps WHERE numeroEtape = 2);
--8 
SELECT c.nomCoureur, CAST(SUM(e.nbKm) / (SUM(EXTRACT(epoch FROM t.tempsRealise)) / 3600) AS DECIMAL(7,4)) AS vitesse_moyenne FROM coureurs c JOIN temps t ON c.numeroCoureur = t.numeroCoureur JOIN etapes e ON t.numeroEtape = e.numeroEtape WHERE t.numeroEtape = (SELECT MAX(numeroEtape) FROM etapes) GROUP BY c.nomCoureur ORDER BY vitesse_moyenne;
--9
SELECT AVG(AGE(dateNaiss)) FROM coureurs;
--10
SELECT COUNT(*) FROM coureurs WHERE AGE(dateNaiss) > (SELECT AVG(AGE(dateNaiss)) FROM coureurs);
--11
SELECT codeEquipe, COUNT(*) FROM coureurs GROUP BY codeEquipe;
--12
SELECT e.codeEquipe, e.nomEquipe, COUNT(*) AS nombre_coureur FROM equipestour e JOIN coureurs c ON e.codeEquipe = c.codeEquipe GROUP BY e.codeEquipe, e.nomEquipe;
--13
SELECT e.codeEquipe, MIN(t.tempsRealise) AS meilleur_temps FROM equipestour e JOIN coureurs c ON e.codeEquipe = c.codeEquipe JOIN temps t ON c.numeroCoureur = t.numeroCoureur WHERE t.numeroEtape = 2 GROUP BY e.codeEquipe;
